...
- **Deploying to production** (not done yet — this was all built and tested locally): deploy `server/` to a host that runs a persistent Node process (Render/Railway/Fly.io/a VPS — not Vercel serverless), set its real `VAPID_*` env vars there (reuse the same keys or regenerate), then update the frontend's `VITE_PUSH_API_URL` to that backend's public URL and redeploy the frontend to Vercel.
