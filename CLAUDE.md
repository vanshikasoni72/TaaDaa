...
- **Two dates per task**: `date` is the "work on" date — it's what drives calendar placement and the Home/Today/Upcoming groupings, unchanged from before. `dueDate` is new — the actual deadline, the harder constraint. "yesterday's problem" (overdue, in both `HomeView` and `TodayView`) keys off `dueDate` now, not `date` — a task can be worked-on today but not overdue, or on-schedule-to-work-on but overdue because the real deadline already passed. Displayed distinctly: `date` renders as plain muted text, `dueDate` renders as a magenta pill (bolder/filled once it's actually passed and the task isn't done). Both are settable from the same calendar-icon popover in the quick-add toolbar and from `TaskEditModal` (two side-by-side date inputs, "Work on" / "Due date").
...
- **Inbox** (`InboxView.tsx`) — tasks with no project assigned, *any* date. Deliberately broad (Todoist convention) rather than "no project AND no date" — a dated-but-unfiled task like "call mom tomorrow" shows in both Inbox and Home/Today, which is intentional; Inbox answers "what haven't I filed anywhere," not "what's totally untouched."
- **Home** (`HomeView.tsx`) is the default landing view: next-7-days tasks grouped by project (sub-category groups are labeled `"Parent / Sub"`, not just `"Sub"`, so the hierarchy stays legible outside the sidebar's nesting), with an overdue section (by `dueDate`, see Data Model) pinned above the groups — this is "Homepage" from the spec, distinct from the **Today** view (`TodayView.tsx`: overdue/today/upcoming/whenever bucketed by `date`, not project).
